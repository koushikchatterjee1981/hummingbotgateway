console.log('entry point reached------5')
import { BlockfrostServerError } from '@blockfrost/blockfrost-js';
import invariant from '@minswap/tiny-invariant';
import Big from 'big.js';
import { Constr, C, getAddressDetails, Data } from 'lucid-cardano';
...
const BATCHER_FEE_REDUCTION_SUPPORTED_ASSET = {
  [NetworkId.MAINNET]: [
...
function getBatcherFee(network, amountMIN, amountLP) {
  ...
  const totalReduction = new BigNumber(activeConfig.maximumReduction).multipliedBy(maximumReductionAmountRatio).div(new BigNumber(100));
  const batcherFee = new BigNumber(1).minus(totalReduction).multipliedBy(new BigNumber(FIXED_BATCHER_FEE.toString())).toFixed(0);
...
export { ADA, Asset, BATCHER_FEE_REDUCTION_SUPPORTED_ASSET, BlockfrostAdapter, DEFAULT_POOL_V2_TRADING_FEE_DENOMINATOR, Dex, DexV1Constant, DexV2Constant, FIXED_DEPOSIT_ADA, MetadataMessage, NetworkId, OrderV1, PoolV1, PoolV2, StablePool, StableswapConstant, calculateDeposit, calculateSwapExactIn, calculateSwapExactOut, calculateWithdraw, calculateZapIn };
