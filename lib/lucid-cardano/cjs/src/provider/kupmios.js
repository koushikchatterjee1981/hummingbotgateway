...
class Kupmios {
  ...
  async getDelegation(rewardAddress) {
    const client = await this.ogmiosWsp("Query", {
      query: {
        "delegationsAndRewards": [rewardAddress]
      }
    });
    return new Promise((res, rej) => {
      client.addEventListener("message", msg => {
        try {
          ...
          res({
            poolId: (delegation === null || delegation === void 0 ? void 0 : delegation.delegate) || null,
            rewards: BigInt((delegation === null || delegation === void 0 ? void 0 : delegation.rewards) || 0)
          });
          client.close();
        } catch (e) {
          rej(e);
    ...
    return new Promise((res, rej) => {
      client.addEventListener("message", msg => {
        try {
          ...
          if (result.SubmitSuccess) res(result.SubmitSuccess.txId);else rej(result.SubmitFail);
          client.close();
        } catch (e) {
          rej(e);
        }
