...
import { ADA_ASSET_DECIMAL, ORDER_DEPOSIT_DEFAULT, V3_POOL_IDENT_LENGTH, } from "../constants.js";
export class SundaeUtils {
    ...
    static getAssetSwapDirection(asset, assets) {
        const sorted = SundaeUtils.sortSwapAssetsWithAmounts([
            new AssetAmount(0n, assets[0]),
            new AssetAmount(0n, assets[1]),
        ]);
    ...
    static getCurrentFeeFromDecayingFee({ endFee, endSlot, network, startFee, startSlot, }) {
        ...
        const interpolatedFee = Fraction.asFraction(openingFee.toNumber() +
    ...
    static getMinReceivableFromSlippage(pool, suppliedAsset, slippage) {
        ...
        const output = getSwapOutput(suppliedAsset.amount, supplyingPoolAssetA ? pool.liquidity.aReserve : pool.liquidity.bReserve, supplyingPoolAssetA ? pool.liquidity.bReserve : pool.liquidity.aReserve, pool.currentFee, false);
        ...
        return new AssetAmount(amount, receivableAssetMetadata);
    }
    /**
        ...
        if (availablePools && resolvedGiven?.metadata && resolvedTaken?.metadata) {
            ...
            for (const pool of availablePools) {
                ...
                const takenReserve = SundaeUtils.isAssetIdsEqual(pool.assetB.assetId, resolvedTaken.metadata.assetId)
                    ? pool.liquidity.bReserve
                    : pool.liquidity.aReserve;
                const swapOutcome = getSwapOutput(hundredGiven, givenReserve, takenReserve, pool.currentFee);
