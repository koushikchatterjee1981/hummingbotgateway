...
import { EContractVersion, EPoolCoin, } from "../../@types/index.js";
...
describe("SundaeUtils class", () => {
    it("getMinReceivableFromSlippage", () => {
        const resultA = SundaeUtils.getMinReceivableFromSlippage(PREVIEW_DATA.pools.v1, PREVIEW_DATA.assets.tada, 0.1);
        expect(resultA).toBeInstanceOf(AssetAmount);
        expect(resultA).toMatchObject(expect.objectContaining({
            amount: 8570604n,
            decimals: 0,
        }));
        const resultB = SundaeUtils.getMinReceivableFromSlippage(PREVIEW_DATA.pools.v1, PREVIEW_DATA.assets.tindy, 0.01);
        expect(resultB).toBeInstanceOf(AssetAmount);
        expect(resultB).toMatchObject(expect.objectContaining({
            amount: 36326909n,
            decimals: 6,
        }));
        expect(() => SundaeUtils.getMinReceivableFromSlippage(PREVIEW_DATA.pools.v1, new AssetAmount(10n, { assetId: "not in the pool", decimals: 0 }), 0.1)).toThrowError(`The supplied asset ID does not match either assets within the supplied pool data. {\"suppliedAssetID\":\"not in the pool\",\"poolAssetIDs\":[\"ada.lovelace\",\"fa3eff2047fdf9293c5feef4dc85ce58097ea1c6da4845a351535183.74494e4459\"]}`);
        /**
        ...
        const mockSuppliedAsset = new AssetAmount(80000000n, PREVIEW_DATA.assets.tada.metadata);
        // Default 3% slippage.
        expect(SundaeUtils.getMinReceivableFromSlippage(mockPoolData, mockSuppliedAsset, 0.3)).toMatchObject(expect.objectContaining({
            amount: 1n,
            decimals: 0,
        }));
        // Even a 99% slippage should be at least a single token.
        expect(SundaeUtils.getMinReceivableFromSlippage(mockPoolData, mockSuppliedAsset, 0.99)).toMatchObject(expect.objectContaining({
            amount: 1n,
            decimals: 0,
        }));
        // Only a 100% slippage will be okay with 0 assets received.
        expect(SundaeUtils.getMinReceivableFromSlippage(mockPoolData, mockSuppliedAsset, 1)).toMatchObject(expect.objectContaining({
            amount: 0n,
            decimals: 0,
        }));
        /**
         ...
         */
        try {
            SundaeUtils.getMinReceivableFromSlippage(mockPoolData, mockSuppliedAsset, 2);
        }
        catch (e) {
    ...
    it("getSwapDirection()", () => {
        ...
        SundaeUtils.ADA_ASSET_IDS.forEach((id) => {
            expect(SundaeUtils.getAssetSwapDirection({
                ...PREVIEW_DATA.assets.tada.metadata,
                assetId: id,
            }, [PREVIEW_DATA.pools.v1.assetA, PREVIEW_DATA.pools.v1.assetB])).toEqual(EPoolCoin.A);
        });
        expect(SundaeUtils.getAssetSwapDirection(PREVIEW_DATA.assets.tindy.metadata, [
    ...
    it("should accurately accumulate suppliedAssets", () => {
        const aggregate = SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets: [
                ...
                new AssetAmount(25000000n, ADA_METADATA),
                PREVIEW_DATA.assets.tindy,
            ],
            scooperFee: 2300000n,
        });
        expect(aggregate).toMatchObject({
            lovelace: 45000000n + 2300000n + ORDER_DEPOSIT_DEFAULT,
    ...
    it("should accurately get the swap direction", () => {
        const result = SundaeUtils.getAssetSwapDirection(PREVIEW_DATA.pools.v1.assetA, [PREVIEW_DATA.pools.v1.assetA, PREVIEW_DATA.pools.v1.assetB]);
        expect(result).toStrictEqual(0);
        const result2 = SundaeUtils.getAssetSwapDirection(PREVIEW_DATA.assets.tindy.metadata, [PREVIEW_DATA.pools.v1.assetA, PREVIEW_DATA.pools.v1.assetB]);
    ...
    it("should correctly convert a decaying pool fee to a Fraction", () => {
        jest.setSystemTime(SundaeUtils.slotToUnix(1702941926 + 200, "preview") * 1000);
        expect(SundaeUtils.getCurrentFeeFromDecayingFee({
            endFee: [1n, 100n],
            endSlot: "1702941979",
            startFee: [1n, 100n],
        ...
        jest.setSystemTime(SundaeUtils.slotToUnix(1702941926 + 1000000, "preview") * 1000);
        expect(SundaeUtils.getCurrentFeeFromDecayingFee({
            endFee: [5n, 100n],
            endSlot: "1712941979",
            startFee: [5n, 1000n],
    ...
    it("should correctly test an assetId as ADA", () => {
        ...
        expect(SundaeUtils.isAdaAsset({ assetId: ".", decimals: 6 })).toBeTruthy();
        expect(SundaeUtils.isAdaAsset({ assetId: "ada.lovelace", decimals: 6 })).toBeTruthy();
        expect(SundaeUtils.isAdaAsset({ assetId: "cardano.ada", decimals: 6 })).toBeTruthy();
        expect(SundaeUtils.isAdaAsset({ assetId: "ada.lovelace", decimals: 4 })).toBeFalsy();
    ...
    it("should return the pool with the best swap outcome", () => {
        ...
        expect(SundaeUtils.getBestPoolBySwapOutcome({
            availablePools: [],
            given,
            taken,
        })).toBeUndefined();
    ...
    describe("isAssetIdsEqual", () => {
        ...
        it("should return true for equal non-ADA asset IDs without periods", () => {
            expect(SundaeUtils.isAssetIdsEqual(nonAdaAssetId1, nonAdaAssetId1)).toBe(true);
        });
        it("should return false for different non-ADA asset IDs", () => {
            expect(SundaeUtils.isAssetIdsEqual(nonAdaAssetId1, nonAdaAssetId2)).toBe(false);
        });
