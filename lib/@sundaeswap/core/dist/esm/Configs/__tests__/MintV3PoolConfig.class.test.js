...
describe("MintV3PoolConfig class", () => {
    ...
    it("should fail when any of the fees surpass the max fee", () => {
        expect(() => new MintV3PoolConfig({
            ...defaultArgs,
            fees: 11000n,
        }).buildArgs()).toThrowError(`Fees cannot supersede the max fee of ${MintV3PoolConfig.MAX_FEE}.`);
        expect(() => new MintV3PoolConfig({
            ...defaultArgs,
            fees: 10n,
        }).buildArgs()).not.toThrowError(`Fees cannot supersede the max fee of ${MintV3PoolConfig.MAX_FEE}.`);
    });
    it("should fail when an ask fee surpasses the max fee", () => {
        ...
        }).buildArgs()).toThrowError(`Ask fee cannot supersede the max fee of ${MintV3PoolConfig.MAX_FEE}.`);
    });
    it("should fail when a bid fee surpasses the max fee", () => {
        ...
        }).buildArgs()).toThrowError(`Bid fee cannot supersede the max fee of ${MintV3PoolConfig.MAX_FEE}.`);
    });
});
