...
import { ADA_METADATA, ORDER_DEPOSIT_DEFAULT, VOID_REDEEMER, } from "../constants.js";
...
export class TxBuilderLucidV1 extends TxBuilder {
    ...
    async getProtocolParams() {
        if (!this.protocolParams) {
            this.protocolParams =
                await this.queryProvider.getProtocolParamsWithScripts(EContractVersion.V1);
        }
    ...
    async swap(swapArgs) {
        ...
        const { inline: cbor } = this.datumBuilder.buildSwapDatum({
            ident,
            swap: {
                SuppliedCoin: SundaeUtils.getAssetSwapDirection(suppliedAsset.metadata, [assetA, assetB]),
        ...
        const v3TxBuilder = new TxBuilderLucidV3(this.lucid, new DatumBuilderLucidV3(this.network));
    ...
    async cancel(cancelArgs) {
        const { utxo, referralFee, ownerAddress } = new CancelConfig(cancelArgs).buildArgs();
        ...
        try {
            ...
            const v3Builder = new TxBuilderLucidV3(this.lucid, new DatumBuilderLucidV3(this.network));
    ...
    async update({ cancelArgs, swapArgs, }) {
         ...
         */
        const { pool: { ident, assetA, assetB }, orderAddresses, suppliedAsset, minReceivable, } = new SwapConfig(swapArgs).buildArgs();
        ...
        cancelTx.payToContract(scriptAddress, swapDatum.inline, SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets: [suppliedAsset],
            scooperFee: this.__getParam("maxScooperFee"),
        }));
        /**
    ...
    async zap(zapArgs) {
        ...
        const minReceivable = SundaeUtils.getMinReceivableFromSlippage(pool, halfSuppliedAmount, swapSlippage);
        let depositPair;
        ...
        const swapData = this.datumBuilder.buildSwapDatum({
            ...
            swap: {
                SuppliedCoin: SundaeUtils.getAssetSwapDirection(suppliedAsset.metadata, [pool.assetA, pool.assetB]),
                MinimumReceivable: minReceivable,
            },
            scooperFee: this.__getParam("maxScooperFee"),
        });
        tx.attachMetadataWithConversion(103251, {
            [`0x${depositHash}`]: SundaeUtils.splitMetadataString(depositInline, "0x"),
        });
    ...
    async migrateLiquidityToV3(migrations, yieldFarming) {
        ...
        const v3TxBuilderInstance = new TxBuilderLucidV3(this.lucid, new DatumBuilderLucidV3(this.network), this.queryProvider);
            ...
            yieldFarming.migrations.forEach(({ withdrawPool, depositPool }) => {
                ...
                const config = {
                    ...
                    withdrawConfig: {
                        ...
                        pool: withdrawPool,
                        suppliedLPAsset: new AssetAmount(migrationAssets[withdrawPool.assetLP.assetId.replace(".", "")].amount, withdrawPool.assetLP),
                    },
                };
        ...
        migrations.forEach(({ withdrawConfig, depositPool }) => {
            ...
            const [coinA, coinB] = getTokensForLp(withdrawArgs.suppliedLPAsset.amount, withdrawArgs.pool.liquidity.aReserve, withdrawArgs.pool.liquidity.bReserve, withdrawArgs.pool.liquidity.lpTotal);
            ...
            metadataDatums[`0x${depositHash}`] = SundaeUtils.splitMetadataString(depositInline, "0x");
        ...
        if (yieldFarming && existingPositionsData) {
            ...
            const signerKey = LucidHelper.getAddressHashes(yieldFarming.ownerAddress.address);
    ...
    async completeTx({ tx, datum, referralFee, deposit, scooperFee, coinSelection, }) {
        const baseFees = {
            ...
            scooperFee: new AssetAmount(scooperFee ?? this.__getParam("maxScooperFee"), ADA_METADATA),
