...
import { C, Data, toUnit, } from "lucid-cardano";
...
import { OrderDatum, SettingsDatum, } from "../DatumBuilders/contracts/contracts.v3.js";
import { V3Types } from "../DatumBuilders/contracts/index.js";
import { QueryProviderSundaeSwap } from "../QueryProviders/QueryProviderSundaeSwap.js";
import { SundaeUtils } from "../Utilities/SundaeUtils.class.js";
import { ADA_METADATA, ORDER_DEPOSIT_DEFAULT, ORDER_ROUTE_DEPOSIT_DEFAULT, POOL_MIN_ADA, VOID_REDEEMER, } from "../constants.js";
...
export class TxBuilderLucidV3 extends TxBuilder {
    ...
    async getProtocolParams() {
        if (!this.protocolParams) {
            this.protocolParams =
                await this.queryProvider.getProtocolParamsWithScripts(EContractVersion.V3);
        }
    ...
    async getMaxScooperFeeAmount() {
        ...
        const { baseFee, simpleFee } = Data.from(settings.datum, SettingsDatum);
    ...
    newTxInstance(fee) {
        ...
        if (fee) {
            const payment = {};
            payment[SundaeUtils.isAdaAsset(fee.payment.metadata)
                ? "lovelace"
                : fee.payment.metadata.assetId] = fee.payment.amount;
            instance.payToAddress(fee.destination, payment);
            if (fee?.feeLabel) {
                instance.attachMetadataWithConversion(674, `${fee.feeLabel}: ${fee.payment.value.toString()} ${!SundaeUtils.isAdaAsset(fee.payment.metadata)
    ...
    async mintPool(mintPoolArgs) {
        ...
        const poolNftNameHex = toUnit(poolPolicyId, DatumBuilderLucidV3.computePoolNftName(newPoolIdent));
        const poolRefNameHex = toUnit(poolPolicyId, DatumBuilderLucidV3.computePoolRefName(newPoolIdent));
        const poolLqNameHex = toUnit(poolPolicyId, DatumBuilderLucidV3.computePoolLqName(newPoolIdent));
        const poolAssets = {
        ...
        const { metadataAdmin: { paymentCredential, stakeCredential }, authorizedStakingKeys: [poolStakingCredential], } = Data.from(settings[0].datum, V3Types.SettingsDatum);
        const metadataAddress = DatumBuilderLucidV3.addressSchemaToBech32({ paymentCredential, stakeCredential }, this.lucid);
        ...
        const tx = this.newTxInstance(referralFee)
            ...
            .payToAddressWithData(metadataAddress, { inline: Data.void() }, {
            lovelace: ORDER_DEPOSIT_DEFAULT,
            [poolRefNameHex]: 1n,
        });
        if (donateToTreasury) {
            const datum = Data.from(settings[0].datum, SettingsDatum);
            const realTreasuryAddress = DatumBuilderLucidV3.addressSchemaToBech32(datum.treasuryAddress, this.lucid);
    ...
    async swap(swapArgs) {
        ...
        const v1TxBUilder = new TxBuilderLucidV1(this.lucid, new DatumBuilderLucidV1(this.network));
    ...
    async orderRouteSwap(args) {
        ...
        if (isSecondSwapV1) {
            tx.attachMetadataWithConversion(103251, {
                [`0x${datumHash}`]: SundaeUtils.splitMetadataString(secondSwapData.datum, "0x"),
            });
        }
        return this.completeTx({
            tx,
    ...
    async cancel(cancelArgs) {
        ...
        try {
            ...
            const v1Builder = new TxBuilderLucidV1(this.lucid, new DatumBuilderLucidV1(this.network));
    ...
    async update({ cancelArgs, swapArgs, }) {
         ...
         */
        const { pool: { ident }, orderAddresses, suppliedAsset, minReceivable, } = new SwapConfig(swapArgs).buildArgs();
        ...
        cancelTx.payToContract(await this.generateScriptAddress("order.spend", orderAddresses.DestinationAddress.address), { inline: swapDatum.inline }, SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets: [suppliedAsset],
            scooperFee: await this.getMaxScooperFeeAmount(),
        }));
        /**
    ...
    async deposit(depositArgs) {
        const { suppliedAssets, pool, orderAddresses, referralFee } = new DepositConfig(depositArgs).buildArgs();
        const tx = this.newTxInstance(referralFee);
        const payment = SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets,
            scooperFee: await this.getMaxScooperFeeAmount(),
        });
        const [coinA, coinB] = SundaeUtils.sortSwapAssetsWithAmounts(suppliedAssets);
    ...
    async withdraw(withdrawArgs) {
        const { suppliedLPAsset, pool, orderAddresses, referralFee } = new WithdrawConfig(withdrawArgs).buildArgs();
        const tx = this.newTxInstance(referralFee);
        const payment = SundaeUtils.accumulateSuppliedAssets({
     ...
     */
    async zap(zapArgs) {
        const zapDirection = SundaeUtils.getAssetSwapDirection(zapArgs.suppliedAsset.metadata, [zapArgs.pool.assetA, zapArgs.pool.assetB]);
        const { pool, suppliedAsset, orderAddresses, swapSlippage, referralFee } = new ZapConfig({
            ...zapArgs,
            zapDirection,
        }).buildArgs();
        const tx = this.newTxInstance(referralFee);
         ...
         */
        const halfSuppliedAmount = new AssetAmount(Math.ceil(Number(suppliedAsset.amount) / 2), suppliedAsset.metadata);
        const minReceivable = SundaeUtils.getMinReceivableFromSlippage(pool, halfSuppliedAmount, swapSlippage ?? 0.3);
        let depositPair;
        ...
        tx.attachMetadataWithConversion(103251, {
            [`0x${depositData.hash}`]: SundaeUtils.splitMetadataString(depositData.inline, "0x"),
        });
        tx.payToContract(await this.generateScriptAddress("order.spend", orderAddresses.DestinationAddress.address), { inline }, payment);
        return this.completeTx({
            tx,
    ...
    async generateScriptAddress(type, ownerAddress) {
        ...
        const paymentStakeCred = C.StakeCredential.from_scripthash(C.ScriptHash.from_hex(hash));
        const ownerStakeCred = ownerAddress &&
            C.Address.from_bech32(ownerAddress).as_base()?.stake_cred();
        if (!ownerStakeCred) {
            return orderAddress;
        }
        const newAddress = C.BaseAddress.new(this.network === "mainnet" ? 1 : 0, paymentStakeCred, ownerStakeCred).to_address();
    ...
    async completeTx({ tx, datum, referralFee, deposit, scooperFee, coinSelection = true, nativeUplc = true, }) {
        const baseFees = {
            ...
            scooperFee: new AssetAmount(scooperFee ?? (await this.getMaxScooperFeeAmount()), ADA_METADATA),
        ...
        const thisTx = {
            ...
            async build() {
                if (!finishedTx) {
                    ...
                    thisTx.fees.cardanoTxFee = new AssetAmount(BigInt(txFee?.to_str() ?? finishedTx?.fee?.toString() ?? "0"), ADA_METADATA);
                }
                return {
