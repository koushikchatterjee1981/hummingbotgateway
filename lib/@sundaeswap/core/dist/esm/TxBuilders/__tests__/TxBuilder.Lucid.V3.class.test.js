...
import { ADA_METADATA, ORDER_DEPOSIT_DEFAULT, POOL_MIN_ADA, } from "../../constants.js";
...
import { mockBlockfrostEvaluateResponse, mockOrderToCancel, params, referenceUtxos, settingsUtxos, } from "./data/mockData.js";
jest
...
const getPaymentAddressFromOutput = (output) => {
    ...
    const paymentAddress = C.EnterpriseAddress.new(0, scriptCredential)
        .to_address()
        .to_bech32("addr_test");
...
describe("TxBuilderLucidV3", () => {
    ...
    it("should get the correct maxScooperFee", async () => {
        ...
        const oldSettingsDatum = Data.from(settingsUtxos[0].datum, SettingsDatum);
    ...
    it("should allow you to cancel an order", async () => {
        ...
        const spiedGetSignerKeyFromDatum = jest.spyOn(DatumBuilderLucidV3, "getSignerKeyFromDatum");
    ...
    test("swap() with incorrect idents should throw", async () => {
        try {
            ...
            expect(e.message).toEqual(DatumBuilderLucidV3.INVALID_POOL_IDENT);
        }
    });
    ...
    test("deposit()", async () => {
        const spiedNewTx = jest.spyOn(builder, "newTxInstance");
        const spiedBuildDepositDatum = jest.spyOn(datumBuilder, "buildDepositDatum");
    ...
    test("withdraw()", async () => {
        ...
        [...Array(builtTx.txComplete.body().outputs().len()).keys()].forEach((index) => {
            ...
            if (getPaymentAddressFromOutput(output) ===
                ...
                output.amount().multiasset() &&
                output.amount().multiasset()?.to_js_value()[PREVIEW_DATA.assets.v3LpToken.metadata.assetId.split(".")[0]][PREVIEW_DATA.assets.v3LpToken.metadata.assetId.split(".")[1]] ===
    ...
    test("mintPool() should build a transaction correctly when including ADA", async () => {
        ...
        expect(fees.deposit.amount.toString()).toEqual((ORDER_DEPOSIT_DEFAULT * 2n).toString());
        const { builtTx } = await build();
        ...
        const poolDepositedAssetB = poolDepositAssets[PREVIEW_DATA.assets.tindy.metadata.assetId.split(".")[0]][PREVIEW_DATA.assets.tindy.metadata.assetId.split(".")[1]];
        ...
        expect(poolDepositedAssetA).toEqual((PREVIEW_DATA.assets.tada.amount + POOL_MIN_ADA).toString());
        ...
        const metadataDepositedNFT = metadataDepositAssets[params.blueprint.validators[1].hash]["000643b09e67cc006063ea055629552650664979d7c92d47e342e5340ef77550"];
        ...
        const lpTokensReturned = lpTokensDepositAssets[params.blueprint.validators[1].hash]["0014df109e67cc006063ea055629552650664979d7c92d47e342e5340ef77550"];
    ...
    test("mintPool() should build a transaction correctly when including ADA and donating Treasury", async () => {
        ...
        expect(Buffer.from(lpTokensOutput.datum()?.as_data_hash()?.to_bytes()).toString("hex")).toEqual(builder.lucid.utils.datumToHash(Data.void()));
        const lpTokensDepositAssets = lpTokensOutput
            .amount()
            .multiasset()
            ?.to_js_value();
    ...
    test("mintPool() should build a transaction correctly when including ADA and donating a percentage to the Treasury", async () => {
        ...
        expect(Buffer.from(lpTokensDonation.datum()?.as_data_hash()?.to_bytes()).toString("hex")).toEqual(builder.lucid.utils.datumToHash(Data.void()));
        const lpTokensDonationAssets = lpTokensDonation
            .amount()
            .multiasset()
            ?.to_js_value();
        const lpTokensDonatedADA = lpTokensDonation.amount().coin().to_str();
        const lpTokensDonated = lpTokensDonationAssets[params.blueprint.validators[1].hash]["0014df109e67cc006063ea055629552650664979d7c92d47e342e5340ef77550"];
    ...
    test("mintPool() should build a transaction correctly when using exotic pairs", async () => {
        ...
        const poolDepositAssets = poolOutput.amount().multiasset()?.to_js_value();
        // const poolDepositedAssetA = poolOutput.amount().coin().to_str();
        const poolDepositedAssetA = poolDepositAssets[PREVIEW_DATA.assets.usdc.metadata.assetId.split(".")[0]][PREVIEW_DATA.assets.usdc.metadata.assetId.split(".")[1]];
        const poolDepositedAssetB = poolDepositAssets[PREVIEW_DATA.assets.tindy.metadata.assetId.split(".")[0]][PREVIEW_DATA.assets.tindy.metadata.assetId.split(".")[1]];
        const poolDepositedNFT = poolDepositAssets["8140c4b89428fc264e90b10c71c53a4c3f9ce52b676bf1d9b51eb9ca"]["000de1409e67cc006063ea055629552650664979d7c92d47e342e5340ef77550"];
        [poolDepositedAssetA, poolDepositedAssetB, poolDepositedNFT].forEach((val) => expect(val).not.toBeUndefined());
        // Should deposit assets without additional ADA deposit.
        expect(poolDepositedAssetA).toEqual(PREVIEW_DATA.assets.tada.amount.toString());
        expect(poolDepositedAssetB).toEqual("20000000");
        expect(poolDepositedNFT).toEqual("1");
        // Should have datum attached.
    ...
    test("mintPool() should throw an error when the ADA provided is less than the min required", async () => {
        ...
        try {
            await builder.mintPool({
                assetA: PREVIEW_DATA.assets.tada.withAmount(500000n),
                assetB: PREVIEW_DATA.assets.usdc,
                fees: 5n,
                marketOpen: 5n,
                ownerAddress: PREVIEW_DATA.addresses.current,
            ...
            expect(e.message).toEqual(TxBuilderLucidV3.MIN_ADA_POOL_MINT_ERROR);
        }
        fetchMock.disableMocks();
    });
