...
describe("buildDestinationAddresses()", () => {
    ...
    it("should fail when passing a script address to DestinationAddress without a datumHash attached", () => {
        ...
        try {
            ...
            expect(LucidHelper.throwInvalidOrderAddressesError).toHaveBeenCalledWith("addr_test1wpesulg5dtt5y73r4zzay9qmy3wnlrxdg944xg4rzuvewls7nrsf0", "The address provided is a Script Address, but a datum hash was not supplied. This will brick your funds! Supply a valid datum hash with the address in order to proceed.");
        }
    });
    it("should fail when passing an invalid datumHash along with a script DestinationAddress", () => {
        ...
        try {
            ...
            expect(LucidHelper.throwInvalidOrderAddressesError).toHaveBeenCalledWith("addr_test1wpesulg5dtt5y73r4zzay9qmy3wnlrxdg944xg4rzuvewls7nrsf0", 'The datum provided was not a valid hex string. Original error: {"datum":{"type":"HASH","value":"invalidDatum"}}');
        }
    });
});
describe("buildOwnerDatum()", () => {
    it("should build the owner datum properly", () => {
        const spiedLucidHelperThrow = jest.spyOn(LucidHelper, "throwInvalidOrderAddressesError");
        const spiedLucidHelperAddressHashes = jest.spyOn(LucidHelper, "getAddressHashes");
        const result = builderInstance.buildOwnerDatum(PREVIEW_DATA.addresses.current);
        expect(spiedLucidHelperAddressHashes).toHaveBeenCalledWith(PREVIEW_DATA.addresses.current);
...
describe("buildAssetDatum", () => {
    it("should correctly build the datum for ADA", () => {
        const result = builderInstance.buildAssetAmountDatum(new AssetAmount(100n, ADA_METADATA));
        expect(result.inline).toEqual("9f40401864ff");
    });
    it("should correctly build the datum for alt-coin", () => {
        const result = builderInstance.buildAssetAmountDatum(new AssetAmount(100000000n, {
            ...ADA_METADATA,
...
describe("static getDestinationAddressesFromDatum()", () => {
    it("should properly extract the addresses from the datum", () => {
        ...
        expect(DatumBuilderLucidV3.getDestinationAddressesFromDatum(exampleDatum)).toMatchObject(expectedAddresses);
    });
});
describe("static addressSchemaToCredential", () => {
    it("should correctly convert address schemas to their Credentials", async () => {
        ...
        expect(DatumBuilderLucidV3.addressSchemaToBech32(paymentAddress, lucidInstance)).toEqual("addr_test1vrp8nglm8d8x9w783c5g0qa4spzaft5z5xyx0kp495p8wkswmc787");
        ...
        expect(DatumBuilderLucidV3.addressSchemaToBech32(paymentAddressWithStakingKey, lucidInstance)).toEqual("addr_test1qrp8nglm8d8x9w783c5g0qa4spzaft5z5xyx0kp495p8wksjrlfzuz6h4ssxlm78v0utlgrhryvl2gvtgp53a6j9zngqtjfk6s");
        const scriptAddress = {
            paymentCredential: {
                SCredential: {
        ...
        expect(DatumBuilderLucidV3.addressSchemaToBech32(scriptAddress, lucidInstance)).toEqual("addr_test1wr866xg5kkvarzll69xjh0tfvqvu9zvuhht2qve9ehmgp0qfgf3wc");
