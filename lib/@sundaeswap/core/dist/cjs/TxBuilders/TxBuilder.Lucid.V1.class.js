...
exports.TxBuilderLucidV1 = void 0;
const asset_1 = require("@sundaeswap/asset");
const cpp_1 = require("@sundaeswap/cpp");
 ...
 */
class TxBuilderLucidV1 extends TxBuilder_abstract_class_js_1.TxBuilder {
    lucid;
    datumBuilder;
    queryProvider;
    network;
    ...
    static PARAMS = {
        mainnet: {
            cancelRedeemer: constants_js_1.VOID_REDEEMER,
            maxScooperFee: 2500000n,
        },
        preview: {
            cancelRedeemer: constants_js_1.VOID_REDEEMER,
            maxScooperFee: 2500000n,
    ...
    async getProtocolParams() {
        if (!this.protocolParams) {
            this.protocolParams =
                await this.queryProvider.getProtocolParamsWithScripts(index_js_1.EContractVersion.V1);
        }
    ...
    async swap(swapArgs) {
        ...
        const { pool: { ident, assetA, assetB }, orderAddresses, suppliedAsset, minReceivable, referralFee, } = config.buildArgs();
        const txInstance = this.newTxInstance(referralFee);
        const { inline: cbor } = this.datumBuilder.buildSwapDatum({
            ident,
            swap: {
                SuppliedCoin: SundaeUtils_class_js_1.SundaeUtils.getAssetSwapDirection(suppliedAsset.metadata, [assetA, assetB]),
        ...
        const v3TxBuilder = new TxBuilder_Lucid_V3_class_js_1.TxBuilderLucidV3(this.lucid, new DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3(this.network));
        const v3Address = await v3TxBuilder.generateScriptAddress("order.spend", swapArgs.ownerAddress ||
    ...
    async orderRouteSwap(args) {
        const isSecondSwapV3 = args.swapB.pool.version === index_js_1.EContractVersion.V3;
        const secondSwapBuilder = isSecondSwapV3
            ? new TxBuilder_Lucid_V3_class_js_1.TxBuilderLucidV3(this.lucid, new DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3(this.network))
            : this;
        const secondSwapAddress = isSecondSwapV3
            ? await secondSwapBuilder.generateScriptAddress("order.spend", args.ownerAddress)
            : await this.getValidatorScript("escrow.spend").then(({ compiledCode }) => this.lucid.utils.validatorToAddress({
                type: "PlutusV1",
                script: compiledCode,
            }));
        ...
        return this.completeTx({
            tx,
            datum,
            deposit: constants_js_1.ORDER_DEPOSIT_DEFAULT,
    ...
    async cancel(cancelArgs) {
        const { utxo, referralFee, ownerAddress } = new CancelConfig_class_js_1.CancelConfig(cancelArgs).buildArgs();
        ...
        try {
            ...
            const v3Builder = new TxBuilder_Lucid_V3_class_js_1.TxBuilderLucidV3(this.lucid, new DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3(this.network));
        ...
        tx.collectFrom(utxosToSpend, this.__getParam("cancelRedeemer")).attachSpendingValidator(scriptValidator);
        const details = (0, lucid_cardano_1.getAddressDetails)(ownerAddress);
        if (details?.stakeCredential?.hash &&
    ...
    async update({ cancelArgs, swapArgs, }) {
         ...
         */
        const { pool: { ident, assetA, assetB }, orderAddresses, suppliedAsset, minReceivable, } = new SwapConfig_class_js_1.SwapConfig(swapArgs).buildArgs();
        ...
        cancelTx.payToContract(scriptAddress, swapDatum.inline, SundaeUtils_class_js_1.SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets: [suppliedAsset],
            scooperFee: this.__getParam("maxScooperFee"),
        }));
        /**
    ...
    async zap(zapArgs) {
        ...
        const minReceivable = SundaeUtils_class_js_1.SundaeUtils.getMinReceivableFromSlippage(pool, halfSuppliedAmount, swapSlippage);
        let depositPair;
         ...
         */
        const { hash: depositHash, inline: depositInline } = this.datumBuilder.buildDepositDatum({
            ident: pool.ident,
            orderAddresses,
            deposit: depositPair,
            scooperFee: this.__getParam("maxScooperFee"),
        ...
        const swapData = this.datumBuilder.buildSwapDatum({
            ...
            swap: {
                SuppliedCoin: SundaeUtils_class_js_1.SundaeUtils.getAssetSwapDirection(suppliedAsset.metadata, [pool.assetA, pool.assetB]),
                MinimumReceivable: minReceivable,
            },
            scooperFee: this.__getParam("maxScooperFee"),
        });
        tx.attachMetadataWithConversion(103251, {
            [`0x${depositHash}`]: SundaeUtils_class_js_1.SundaeUtils.splitMetadataString(depositInline, "0x"),
        });
     ...
     * ```
     */
    async migrateLiquidityToV3(migrations, yieldFarming) {
        const finalTx = this.lucid.newTx();
        let totalScooper = 0n;
        let totalDeposit = 0n;
        let totalReferralFees = new asset_1.AssetAmount(0n, constants_js_1.ADA_METADATA);
        const metadataDatums = {};
        const v3TxBuilderInstance = new TxBuilder_Lucid_V3_class_js_1.TxBuilderLucidV3(this.lucid, new DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3(this.network), this.queryProvider);
        const v3OrderScriptAddress = await v3TxBuilderInstance.generateScriptAddress("order.spend");
        ...
        const YF_V2_PARAMS = {
            mainnet: {
                ...
                referenceInput: "006ddd85cfc2e2d8b7238daa37b37a5db2ac63de2df35884a5e501667981e1e3#0",
            },
            preview: {
                stakeKeyHash: "045d47cac5067ce697478c11051deb935a152e0773a5d7430a11baa8",
                scriptHash: "73275b9e267fd927bfc14cf653d904d1538ad8869260ab638bf73f5c",
                referenceInput: "aaaf193b8418253f4169ab869b77dedd4ee3df4f2837c226cee3c2f7fa955189#0",
            },
        };
        const yfRefInput = yieldFarming &&
            (await this.lucid.provider.getUtxosByOutRef([
                {
                    txHash: YF_V2_PARAMS[this.network].referenceInput.split("#")[0],
                    outputIndex: Number(YF_V2_PARAMS[this.network].referenceInput.split("#")[1]),
                },
            ]));
        const existingPositionsData = yieldFarming?.existingPositions &&
            (await this.lucid.provider.getUtxosByOutRef(yieldFarming.existingPositions.map(({ hash, index }) => ({
                outputIndex: index,
                txHash: hash,
            }))));
        ...
        if (yieldFarming &&
            ...
            finalTx
                .readFrom(yfRefInput)
                .collectFrom(existingPositionsData, constants_js_1.VOID_REDEEMER);
            const withdrawAssetsList = yieldFarming.migrations.reduce((list, { withdrawPool }) => {
            ...
            if (Object.keys(migrationAssets).length === 0) {
                throw new Error("There were no eligible assets to migrate within the provided existing positions. Please check your migration config, and try again.");
            }
                    ...
                    withdrawConfig: {
                        orderAddresses: {
                            DestinationAddress: {
                                ...
                                datum: {
                                    type: index_js_1.EDatumType.INLINE,
                                    value: oldDelegation.datum,
                                },
                            },
                            AlternateAddress: yieldFarming.ownerAddress.address,
                        },
                        pool: withdrawPool,
                        suppliedLPAsset: new asset_1.AssetAmount(migrationAssets[withdrawPool.assetLP.assetId.replace(".", "")].amount, withdrawPool.assetLP),
                    },
                };
        ...
        migrations.forEach(({ withdrawConfig, depositPool }) => {
            const withdrawArgs = new WithdrawConfig_class_js_1.WithdrawConfig(withdrawConfig).buildArgs();
            ...
            totalDeposit += constants_js_1.ORDER_DEPOSIT_DEFAULT;
            totalScooper += scooperFee;
            withdrawArgs.referralFee?.payment &&
                totalReferralFees.add(withdrawArgs.referralFee.payment);
            const [coinA, coinB] = (0, cpp_1.getTokensForLp)(withdrawArgs.suppliedLPAsset.amount, withdrawArgs.pool.liquidity.aReserve, withdrawArgs.pool.liquidity.bReserve, withdrawArgs.pool.liquidity.lpTotal);
            const v3DatumBuilder = new DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3(this.network);
            const { hash: depositHash, inline: depositInline } = v3DatumBuilder.buildDepositDatum({
                ...
                order: {
                    assetA: new asset_1.AssetAmount(coinA, depositPool.assetA),
                    assetB: new asset_1.AssetAmount(coinB, depositPool.assetB),
                },
            ...
            metadataDatums[`0x${depositHash}`] = SundaeUtils_class_js_1.SundaeUtils.splitMetadataString(depositInline, "0x");
        ...
        if (yieldFarming && existingPositionsData) {
            ...
            const signerKey = LucidHelper_class_js_1.LucidHelper.getAddressHashes(yieldFarming.ownerAddress.address);
            ...
            finalTx.payToContract(lockContractAddress, { inline: existingPositionsData[0].datum }, returningPayment);
        }
    ...
    async completeTx({ tx, datum, referralFee, deposit, scooperFee, coinSelection, }) {
        const baseFees = {
            deposit: new asset_1.AssetAmount(deposit ?? constants_js_1.ORDER_DEPOSIT_DEFAULT, constants_js_1.ADA_METADATA),
            scooperFee: new asset_1.AssetAmount(scooperFee ?? this.__getParam("maxScooperFee"), constants_js_1.ADA_METADATA),
...
exports.TxBuilderLucidV1 = TxBuilderLucidV1;
//# sourceMappingURL=TxBuilder.Lucid.V1.class.js.map