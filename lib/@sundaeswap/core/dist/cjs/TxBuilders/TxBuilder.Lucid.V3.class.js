...
exports.TxBuilderLucidV3 = void 0;
const asset_1 = require("@sundaeswap/asset");
const lucid_cardano_1 = require("lucid-cardano");
const index_js_1 = require("../@types/index.js");
const TxBuilder_abstract_class_js_1 = require("../Abstracts/TxBuilder.abstract.class.js");
const CancelConfig_class_js_1 = require("../Configs/CancelConfig.class.js");
const DepositConfig_class_js_1 = require("../Configs/DepositConfig.class.js");
const MintV3PoolConfig_class_js_1 = require("../Configs/MintV3PoolConfig.class.js");
const SwapConfig_class_js_1 = require("../Configs/SwapConfig.class.js");
const WithdrawConfig_class_js_1 = require("../Configs/WithdrawConfig.class.js");
const ZapConfig_class_js_1 = require("../Configs/ZapConfig.class.js");
...
const index_js_2 = require("../DatumBuilders/contracts/index.js");
 ...
 */
class TxBuilderLucidV3 extends TxBuilder_abstract_class_js_1.TxBuilder {
    lucid;
    datumBuilder;
    queryProvider;
    network;
    ...
    settingsUtxos;
    validatorScripts = {};
    static MIN_ADA_POOL_MINT_ERROR = "You tried to create a pool with less ADA than is required. Try again with more than 2 ADA.";
    SETTINGS_NFT_NAME = "73657474696e6773";
    /**
     ...
     */
    constructor(lucid, datumBuilder, queryProvider) {
        super();
        this.lucid = lucid;
        this.datumBuilder = datumBuilder;
        this.network = lucid.network === "Mainnet" ? "mainnet" : "preview";
        this.queryProvider =
            queryProvider ?? new QueryProviderSundaeSwap_js_1.QueryProviderSundaeSwap(this.network);
    }
    /**
    ...
    async getProtocolParams() {
        if (!this.protocolParams) {
            this.protocolParams =
                await this.queryProvider.getProtocolParamsWithScripts(index_js_1.EContractVersion.V3);
        }
     ...
     */
    async getAllReferenceUtxos() {
        if (!this.referenceUtxos) {
            const utxos = [];
            const { references } = await this.getProtocolParams();
            references.forEach(({ txIn }) => utxos.push({ outputIndex: txIn.index, txHash: txIn.hash }));
     ...
     * @returns {Promise<UTxO[]>}
     */
    async getAllSettingsUtxos() {
        if (!this.settingsUtxos) {
            const { hash } = await this.getValidatorScript("settings.mint");
            this.settingsUtxos = [
                await this.lucid.provider.getUtxoByUnit(`${hash}${this.SETTINGS_NFT_NAME}`),
            ];
        }
     ...
     */
    async getMaxScooperFeeAmount() {
        const [settings] = await this.getAllSettingsUtxos();
        if (!settings) {
            return 1000000n;
        }
        const { baseFee, simpleFee } = lucid_cardano_1.Data.from(settings.datum, contracts_v3_js_1.SettingsDatum);
     ...
     */
    async getValidatorScript(name) {
        const params = await this.getProtocolParams();
        const result = params.blueprint.validators.find(({ title }) => title === name);
        if (!result) {
    ...
    newTxInstance(fee) {
        ...
        if (fee) {
            const payment = {};
            payment[SundaeUtils_class_js_1.SundaeUtils.isAdaAsset(fee.payment.metadata)
                ? "lovelace"
                : fee.payment.metadata.assetId] = fee.payment.amount;
            instance.payToAddress(fee.destination, payment);
            if (fee?.feeLabel) {
                instance.attachMetadataWithConversion(674, `${fee.feeLabel}: ${fee.payment.value.toString()} ${!SundaeUtils_class_js_1.SundaeUtils.isAdaAsset(fee.payment.metadata)
                    ? Buffer.from(fee.payment.metadata.assetId.split(".")[1], "hex").toString("utf-8")
                    : "ADA"}`);
            }
        }
        return instance;
     ...
     */
    async mintPool(mintPoolArgs) {
        const { assetA, assetB, fees, marketOpen, ownerAddress, referralFee, donateToTreasury, } = new MintV3PoolConfig_class_js_1.MintV3PoolConfig(mintPoolArgs).buildArgs();
        const sortedAssets = SundaeUtils_class_js_1.SundaeUtils.sortSwapAssetsWithAmounts([
            assetA,
            assetB,
        ]);
        const exoticPair = !SundaeUtils_class_js_1.SundaeUtils.isAdaAsset(sortedAssets[0].metadata);
        const [userUtxos, { hash: poolPolicyId }, references, settings] = await Promise.all([
            this.getUtxosForPoolMint(),
            this.getValidatorScript("pool.mint"),
            this.getAllReferenceUtxos(),
            this.getAllSettingsUtxos(),
        ]);
        const newPoolIdent = DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.computePoolId(userUtxos[0]);
        const poolNftNameHex = (0, lucid_cardano_1.toUnit)(poolPolicyId, DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.computePoolNftName(newPoolIdent));
        const poolRefNameHex = (0, lucid_cardano_1.toUnit)(poolPolicyId, DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.computePoolRefName(newPoolIdent));
        const poolLqNameHex = (0, lucid_cardano_1.toUnit)(poolPolicyId, DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.computePoolLqName(newPoolIdent));
        const poolAssets = {
            lovelace: constants_js_1.POOL_MIN_ADA,
            [poolNftNameHex]: 1n,
            [sortedAssets[1].metadata.assetId.replace(".", "")]: sortedAssets[1].amount,
        };
        if (exoticPair) {
            // Add non-ada asset.
        ...
        const { inline: mintPoolDatum, schema: { circulatingLp }, } = this.datumBuilder.buildMintPoolDatum({
            assetA: sortedAssets[0],
            assetB: sortedAssets[1],
            fees,
            marketOpen,
            depositFee: constants_js_1.POOL_MIN_ADA,
            seedUtxo: userUtxos[0],
        });
        const { inline: mintRedeemerDatum } = this.datumBuilder.buildPoolMintRedeemerDatum({
            assetA: sortedAssets[0],
            assetB: sortedAssets[1],
        ...
        const { metadataAdmin: { paymentCredential, stakeCredential }, authorizedStakingKeys: [poolStakingCredential], } = lucid_cardano_1.Data.from(settings[0].datum, index_js_2.V3Types.SettingsDatum);
        const metadataAddress = DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.addressSchemaToBech32({ paymentCredential, stakeCredential }, this.lucid);
        const { blueprint } = await this.getProtocolParams();
        const poolContract = blueprint.validators.find(({ title }) => title === "pool.mint");
        const sundaeStakeAddress = DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.addressSchemaToBech32({
            paymentCredential: {
                SCredential: {
                    bytes: poolContract?.hash,
                },
            },
            stakeCredential: {
        ...
        const tx = this.newTxInstance(referralFee)
            ...
            .payToAddressWithData(metadataAddress, { inline: lucid_cardano_1.Data.void() }, {
            lovelace: constants_js_1.ORDER_DEPOSIT_DEFAULT,
            [poolRefNameHex]: 1n,
        });
        if (donateToTreasury) {
            const datum = lucid_cardano_1.Data.from(settings[0].datum, contracts_v3_js_1.SettingsDatum);
            const realTreasuryAddress = DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.addressSchemaToBech32(datum.treasuryAddress, this.lucid);
            if (donateToTreasury === 100n) {
                tx.payToContract(realTreasuryAddress, lucid_cardano_1.Data.void(), {
                    lovelace: constants_js_1.ORDER_DEPOSIT_DEFAULT,
                    [poolLqNameHex]: circulatingLp,
                });
            }
        ...
        return this.completeTx({
            ...
            deposit: constants_js_1.ORDER_DEPOSIT_DEFAULT * (exoticPair ? 3n : 2n),
            /**
     ...
     */
    async swap(swapArgs) {
        const config = new SwapConfig_class_js_1.SwapConfig(swapArgs);
        const { pool: { ident }, orderAddresses, suppliedAsset, minReceivable, referralFee, } = config.buildArgs();
        ...
        const v1TxBUilder = new TxBuilder_Lucid_V1_class_js_1.TxBuilderLucidV1(this.lucid, new DatumBuilder_Lucid_V1_class_js_1.DatumBuilderLucidV1(this.network));
        const v1Address = await v1TxBUilder
            .getValidatorScript("escrow.spend")
            .then(({ compiledCode }) => this.lucid.utils.validatorToAddress({
            type: "PlutusV1",
            script: compiledCode,
        }));
        const v3Address = await this.generateScriptAddress("order.spend", swapArgs?.ownerAddress ?? orderAddresses.DestinationAddress.address);
        ...
        const isOrderRoute = [v1Address, v3Address].includes(orderAddresses.DestinationAddress.address);
        const payment = SundaeUtils_class_js_1.SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets: [suppliedAsset],
            scooperFee,
            orderDeposit: isOrderRoute
                ? constants_js_1.ORDER_ROUTE_DEPOSIT_DEFAULT
                : constants_js_1.ORDER_DEPOSIT_DEFAULT,
        });
    ...
    async orderRouteSwap(args) {
        const isSecondSwapV1 = args.swapB.pool.version === index_js_1.EContractVersion.V1;
        const secondSwapBuilder = isSecondSwapV1
            ? new TxBuilder_Lucid_V1_class_js_1.TxBuilderLucidV1(this.lucid, new DatumBuilder_Lucid_V1_class_js_1.DatumBuilderLucidV1(this.network))
            : this;
        ...
        const swapA = new SwapConfig_class_js_1.SwapConfig({
            ...args.swapA,
        ...
        const [aReserve, bReserve] = SundaeUtils_class_js_1.SundaeUtils.sortSwapAssetsWithAmounts([
            new asset_1.AssetAmount(args.swapA.pool.liquidity.aReserve, args.swapA.pool.assetA),
            new asset_1.AssetAmount(args.swapA.pool.liquidity.bReserve, args.swapA.pool.assetB),
        ]);
        const aOutputAsset = swapA.suppliedAsset.metadata.assetId === aReserve.metadata.assetId
            ? bReserve.withAmount(swapA.minReceivable.amount)
            : aReserve.withAmount(swapA.minReceivable.amount);
        const swapB = new SwapConfig_class_js_1.SwapConfig({
            ...args.swapB,
        ...
        const datumHash = this.lucid.utils.datumToHash(secondSwapData.datum);
        const { tx, datum, fees } = await this.swap({
            ...
            orderAddresses: {
                ...
                DestinationAddress: {
                    ...
                    datum: {
                        type: isSecondSwapV1 ? index_js_1.EDatumType.HASH : index_js_1.EDatumType.INLINE,
                        value: isSecondSwapV1
                            ? datumHash
                            : secondSwapData.datum,
                    },
                },
        ...
        if (isSecondSwapV1) {
            tx.attachMetadataWithConversion(103251, {
                [`0x${datumHash}`]: SundaeUtils_class_js_1.SundaeUtils.splitMetadataString(secondSwapData.datum, "0x"),
            });
        }
        return this.completeTx({
            tx,
            datum,
            deposit: constants_js_1.ORDER_ROUTE_DEPOSIT_DEFAULT,
     ...
     */
    async cancel(cancelArgs) {
        const { utxo, referralFee } = new CancelConfig_class_js_1.CancelConfig(cancelArgs).buildArgs();
        const tx = this.newTxInstance(referralFee);
        const utxosToSpend = await this.lucid.provider.getUtxosByOutRef([
            { outputIndex: utxo.index, txHash: utxo.hash },
        ]);
        if (!utxosToSpend) {
            throw new Error(`UTXO data was not found with the following parameters: ${JSON.stringify(utxo)}`);
        }
        const spendingDatum = utxosToSpend[0]?.datum ||
            (utxosToSpend[0]?.datumHash &&
                (await this.lucid.provider.getDatum(utxosToSpend[0].datumHash)));
        if (!spendingDatum) {
            throw new Error("Failed trying to cancel an order that doesn't include a datum!");
        }
        /**
         ...
         */
        try {
            lucid_cardano_1.Data.from(spendingDatum, contracts_v3_js_1.OrderDatum);
        }
        catch (e) {
            console.log("This is a V1 order! Calling appropriate builder...");
            const v1Builder = new TxBuilder_Lucid_V1_class_js_1.TxBuilderLucidV1(this.lucid, new DatumBuilder_Lucid_V1_class_js_1.DatumBuilderLucidV1(this.network));
        ...
        tx.collectFrom(utxosToSpend, constants_js_1.VOID_REDEEMER).readFrom(cancelReadFrom);
        const signerKey = DatumBuilder_Lucid_V3_class_js_1.DatumBuilderLucidV3.getSignerKeyFromDatum(spendingDatum);
        if (signerKey) {
     ...
     */
    async update({ cancelArgs, swapArgs, }) {
        /**
         ...
         */
        const { pool: { ident }, orderAddresses, suppliedAsset, minReceivable, } = new SwapConfig_class_js_1.SwapConfig(swapArgs).buildArgs();
        ...
        cancelTx.payToContract(await this.generateScriptAddress("order.spend", orderAddresses.DestinationAddress.address), { inline: swapDatum.inline }, SundaeUtils_class_js_1.SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets: [suppliedAsset],
            scooperFee: await this.getMaxScooperFeeAmount(),
        }));
        /**
        ...
        if (swapArgs?.referralFee) {
            ...
            cancelTx.payToAddress(swapArgs.referralFee.destination, {
                [SundaeUtils_class_js_1.SundaeUtils.isAdaAsset(swapArgs.referralFee.payment.metadata)
                    ? "lovelace"
                    : swapArgs.referralFee.payment.metadata.assetId]: swapArgs.referralFee.payment.amount,
            });
        }
        return this.completeTx({
            tx: cancelTx,
     ...
     */
    async deposit(depositArgs) {
        const { suppliedAssets, pool, orderAddresses, referralFee } = new DepositConfig_class_js_1.DepositConfig(depositArgs).buildArgs();
        const tx = this.newTxInstance(referralFee);
        const payment = SundaeUtils_class_js_1.SundaeUtils.accumulateSuppliedAssets({
            suppliedAssets,
            scooperFee: await this.getMaxScooperFeeAmount(),
        });
        const [coinA, coinB] = SundaeUtils_class_js_1.SundaeUtils.sortSwapAssetsWithAmounts(suppliedAssets);
        ...
        tx.payToContract(await this.generateScriptAddress("order.spend", orderAddresses.DestinationAddress.address), { inline }, payment);
        return this.completeTx({
            tx,
            datum: inline,
            referralFee: referralFee?.payment,
     ...
     * @returns {Promise<IComposedTx<Tx, TxComplete, Datum | undefined>>} A promise that resolves to the composed transaction object.
     */
    async withdraw(withdrawArgs) {
        const { suppliedLPAsset, pool, orderAddresses, referralFee } = new WithdrawConfig_class_js_1.WithdrawConfig(withdrawArgs).buildArgs();
        const tx = this.newTxInstance(referralFee);
        const payment = SundaeUtils_class_js_1.SundaeUtils.accumulateSuppliedAssets({
     ...
     */
    async zap(zapArgs) {
        const zapDirection = SundaeUtils_class_js_1.SundaeUtils.getAssetSwapDirection(zapArgs.suppliedAsset.metadata, [zapArgs.pool.assetA, zapArgs.pool.assetB]);
        const { pool, suppliedAsset, orderAddresses, swapSlippage, referralFee } = new ZapConfig_class_js_1.ZapConfig({
            ...zapArgs,
            zapDirection,
        }).buildArgs();
        const tx = this.newTxInstance(referralFee);
         ...
         */
        const halfSuppliedAmount = new asset_1.AssetAmount(Math.ceil(Number(suppliedAsset.amount) / 2), suppliedAsset.metadata);
        const minReceivable = SundaeUtils_class_js_1.SundaeUtils.getMinReceivableFromSlippage(pool, halfSuppliedAmount, swapSlippage ?? 0.3);
        let depositPair;
        if (SundaeUtils_class_js_1.SundaeUtils.isAssetIdsEqual(pool.assetA.assetId, suppliedAsset.metadata.assetId)) {
            depositPair = {
        ...
        if (!depositData?.hash) {
            throw new Error("A datum hash for a deposit transaction is required to build a chained Zap operation.");
        }
        /**
        ...
        const { inline } = this.datumBuilder.buildSwapDatum({
            ident: pool.ident,
            destinationAddress: {
                address: await this.generateScriptAddress("order.spend", orderAddresses.DestinationAddress.address),
                /**
                 * @TODO
        ...
        tx.attachMetadataWithConversion(103251, {
            [`0x${depositData.hash}`]: SundaeUtils_class_js_1.SundaeUtils.splitMetadataString(depositData.inline, "0x"),
        });
        tx.payToContract(await this.generateScriptAddress("order.spend", orderAddresses.DestinationAddress.address), { inline }, payment);
        return this.completeTx({
            tx,
     ...
     */
    async generateScriptAddress(type, ownerAddress) {
        ...
        const paymentStakeCred = lucid_cardano_1.C.StakeCredential.from_scripthash(lucid_cardano_1.C.ScriptHash.from_hex(hash));
        const ownerStakeCred = ownerAddress &&
            lucid_cardano_1.C.Address.from_bech32(ownerAddress).as_base()?.stake_cred();
        if (!ownerStakeCred) {
            return orderAddress;
        }
        const newAddress = lucid_cardano_1.C.BaseAddress.new(this.network === "mainnet" ? 1 : 0, paymentStakeCred, ownerStakeCred).to_address();
        return newAddress.to_bech32(this.network === "mainnet" ? "addr" : "addr_test");
    }
    /**
    ...
    async completeTx({ tx, datum, referralFee, deposit, scooperFee, coinSelection = true, nativeUplc = true, }) {
        const baseFees = {
            deposit: new asset_1.AssetAmount(deposit ?? constants_js_1.ORDER_DEPOSIT_DEFAULT, constants_js_1.ADA_METADATA),
            scooperFee: new asset_1.AssetAmount(scooperFee ?? (await this.getMaxScooperFeeAmount()), constants_js_1.ADA_METADATA),
        ...
        const thisTx = {
            ...
            async build() {
                if (!finishedTx) {
                    ...
                    thisTx.fees.cardanoTxFee = new asset_1.AssetAmount(BigInt(txFee?.to_str() ?? finishedTx?.fee?.toString() ?? "0"), constants_js_1.ADA_METADATA);
                }
                return {
                        ...
                        return {
                            ...
                            submit: async () => {
                                try {
                                    ...
                                    console.log(`Could not submit order. Signed transaction CBOR: ${Buffer.from(signedTx.txSigned.body().to_bytes()).toString("hex")}`);
                                    throw e;
                                }
                            },
                        };
...
exports.TxBuilderLucidV3 = TxBuilderLucidV3;
//# sourceMappingURL=TxBuilder.Lucid.V3.class.js.map