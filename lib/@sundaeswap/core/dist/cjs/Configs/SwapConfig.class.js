 ...
 * ```
 */
class SwapConfig extends OrderConfig_abstract_class_js_1.OrderConfig {
    suppliedAsset;
    minReceivable;
    constructor(args) {
        super();
     ...
     */
    setFromObject({ pool, orderAddresses, suppliedAsset, referralFee, swapType, }) {
        this.setPool(pool);
        ...
        switch (swapType.type) {
            case index_js_1.ESwapType.MARKET:
                this.setMinReceivable(SundaeUtils_class_js_1.SundaeUtils.getMinReceivableFromSlippage(pool, suppliedAsset, swapType.slippage));
                break;
            case index_js_1.ESwapType.LIMIT:
    ...
    validate() {
        ...
        if (!this.suppliedAsset) {
            throw new Error("You haven't funded this swap on your SwapConfig! Fund the swap with .setSuppliedAsset()");
        }
        if (!this.minReceivable) {
            throw new Error("A minimum receivable amount was not found. This is usually because an invalid swapType was not supplied in the config.");
        }
        if (this.minReceivable.amount < 0n) {
            throw new Error("Cannot use a negative minimum receivable amount. Please try again.");
        }
    }
}
exports.SwapConfig = SwapConfig;
//# sourceMappingURL=SwapConfig.class.js.map