...
exports.DatumBuilderLucidV3 = void 0;
const bigint_math_1 = require("@sundaeswap/bigint-math");
...
const V3Types = __importStar(require("./contracts/contracts.v3.js"));
/**
...
class DatumBuilderLucidV3 {
    ...
    buildSwapDatum({ destinationAddress, ident, order, ownerAddress, scooperFee, }) {
        const datum = {
            ...
            extension: lucid_cardano_1.Data.void(),
        };
        const inline = lucid_cardano_1.Data.to(datum, V3Types.OrderDatum);
        return {
    ...
    buildMintPoolDatum({ assetA, assetB, fees, marketOpen, depositFee, seedUtxo, }) {
        ...
        const liquidity = (0, bigint_math_1.sqrt)(assetA.amount * assetB.amount);
        ...
        const inline = lucid_cardano_1.Data.to(newPoolDatum, V3Types.PoolDatum);
        return {
            hash: LucidHelper_class_js_1.LucidHelper.inlineDatumToHash(inline),
            inline,
            schema: newPoolDatum,
    ...
    buildPoolMintRedeemerDatum({ assetA, assetB, metadataOutput, poolOutput, }) {
        ...
        const inline = lucid_cardano_1.Data.to(poolMintRedeemer, V3Types.PoolMintRedeemer);
        return {
            hash: LucidHelper_class_js_1.LucidHelper.inlineDatumToHash(inline),
            inline,
    ...
    buildDestinationAddresses({ address, datum, }) {
        ...
        const destination = LucidHelper_class_js_1.LucidHelper.getAddressHashes(address);
        let formattedDatum;
        switch (datum.type) {
            case index_js_1.EDatumType.NONE:
                formattedDatum = new lucid_cardano_1.Constr(0, []);
                break;
            case index_js_1.EDatumType.HASH:
                formattedDatum = new lucid_cardano_1.Constr(1, [datum.value]);
                break;
            case index_js_1.EDatumType.INLINE:
                formattedDatum = new lucid_cardano_1.Constr(2, [lucid_cardano_1.Data.from(datum.value)]);
                break;
            default:
        ...
        const destinationDatum = {
            address: {
                paymentCredential: LucidHelper_class_js_1.LucidHelper.isScriptAddress(address)
                    ? {
                        SCredential: {
        ...
        const inline = lucid_cardano_1.Data.to(destinationDatum, V3Types.Destination);
        return {
            hash: LucidHelper_class_js_1.LucidHelper.inlineDatumToHash(inline),
            inline,
    ...
    buildOwnerDatum(main) {
        LucidHelper_class_js_1.LucidHelper.validateAddressNetwork(main, this.network);
        const { stakeCredentials, paymentCredentials } = LucidHelper_class_js_1.LucidHelper.getAddressHashes(main);
        const ownerDatum = {
            Address: { hex: stakeCredentials || paymentCredentials },
        };
        const inline = lucid_cardano_1.Data.to(ownerDatum, V3Types.MultiSigScript);
        return {
            hash: LucidHelper_class_js_1.LucidHelper.inlineDatumToHash(inline),
            inline,
            schema: ownerDatum,
    ...
    buildAssetAmountDatum(asset) {
        const isAdaLovelace = SundaeUtils_class_js_1.SundaeUtils.isAdaAsset(asset.metadata);
        const value = [
            isAdaLovelace ? "" : asset.metadata.assetId.split(".")[0],
            isAdaLovelace ? "" : asset.metadata.assetId.split(".")[1],
            asset.amount,
        ];
        const inline = lucid_cardano_1.Data.to(value, V3Types.SingletonValue);
        return {
            hash: LucidHelper_class_js_1.LucidHelper.inlineDatumToHash(inline),
            inline,
            schema: value,
    ...
    buildLexicographicalAssetsDatum(assetA, assetB) {
        const lexicographicalAssets = SundaeUtils_class_js_1.SundaeUtils.sortSwapAssetsWithAmounts([
            assetA,
            assetB,
        ]);
        const assets = lexicographicalAssets.reduce((result, { metadata }) => {
            if (SundaeUtils_class_js_1.SundaeUtils.isAdaAsset(metadata)) {
                result.push(["", ""]);
                return result;
            }
        ...
        const inline = lucid_cardano_1.Data.to(assets, V3Types.AssetClassPair);
        return {
            hash: LucidHelper_class_js_1.LucidHelper.inlineDatumToHash(inline),
            inline,
            schema: assets,
    ...
    buildPoolIdent(ident) {
        if (ident.length !== constants_js_1.V3_POOL_IDENT_LENGTH) {
    ...
    static computePoolId(seed) {
        ...
        const hash = Buffer.from(lucid_cardano_1.C.hash_blake2b256(poolInputRef)
    ...
    static getDestinationAddressesFromDatum(datum) {
        const { destination: { address }, } = lucid_cardano_1.Data.from(datum, V3Types.OrderDatum);
        let stakingKeyHash;
        let paymentKeyHash;
    ...
    static getSignerKeyFromDatum(datum) {
        const { owner } = lucid_cardano_1.Data.from(datum, V3Types.OrderDatum);
...
exports.DatumBuilderLucidV3 = DatumBuilderLucidV3;
