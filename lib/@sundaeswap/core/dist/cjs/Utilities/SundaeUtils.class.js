...
class SundaeUtils {
     ...
     * @returns {boolean}
     */
    static isAdaAsset(asset) {
        if (!asset ||
            !SundaeUtils.ADA_ASSET_IDS.includes(asset.assetId) ||
            asset.decimals !== constants_js_1.ADA_ASSET_DECIMAL) {
            return false;
        }
        return true;
    }
     ...
     */
    static isV3PoolIdent(poolIdent) {
        return poolIdent.length === constants_js_1.V3_POOL_IDENT_LENGTH;
    }
    /**
    ...
    static isLPAsset({ assetPolicyId, protocols, version, }) {
        ...
        const validator = protocol?.blueprint.validators.find((v) => v.title === "pool.mint");
    ...
    static getAssetSwapDirection(asset, assets) {
        const sorted = SundaeUtils.sortSwapAssetsWithAmounts([
            new asset_1.AssetAmount(0n, assets[0]),
            new asset_1.AssetAmount(0n, assets[1]),
        ]);
        if (SundaeUtils.isAssetIdsEqual(sorted[1]?.metadata.assetId, asset.assetId)) {
            return 1;
        }
        return 0;
    }
     ...
     * @returns
     */
    static subtractPoolFeeFromAmount(amount, fee) {
        const percent = new fraction_1.Fraction(fee[0], fee[1]).toNumber();
        return Number(amount.amount) * (1 - percent);
    }
    static getCurrentFeeFromDecayingFee({ endFee, endSlot, network, startFee, startSlot, }) {
        ...
        const openingFee = new fraction_1.Fraction(...startFee);
        const finalFee = new fraction_1.Fraction(...endFee);
        ...
        const interpolatedFee = fraction_1.Fraction.asFraction(openingFee.toNumber() +
     ...
     * ```
     */
    static getMinReceivableFromSlippage(pool, suppliedAsset, slippage) {
        const supplyingPoolAssetA = SundaeUtils.isAssetIdsEqual(pool.assetA.assetId, suppliedAsset.metadata.assetId);
        const output = (0, cpp_1.getSwapOutput)(suppliedAsset.amount, supplyingPoolAssetA ? pool.liquidity.aReserve : pool.liquidity.bReserve, supplyingPoolAssetA ? pool.liquidity.bReserve : pool.liquidity.aReserve, pool.currentFee, false);
        if (!SundaeUtils.isAssetIdsEqual(pool.assetA.assetId, suppliedAsset.metadata.assetId) &&
            !SundaeUtils.isAssetIdsEqual(pool.assetB.assetId, suppliedAsset.metadata.assetId)) {
            throw new Error(`The supplied asset ID does not match either assets within the supplied pool data. ${JSON.stringify({
        ...
        return new asset_1.AssetAmount(amount, receivableAssetMetadata);
    }
    /**
     ...
     * @returns {boolean}
     */
    static isAssetIdsEqual(aId, bId) {
        if (SundaeUtils.ADA_ASSET_IDS.includes(aId) &&
    ...
    static accumulateSuppliedAssets({ scooperFee, suppliedAssets, orderDeposit, }) {
        ...
        const aggregatedAssets = suppliedAssets.reduce((acc, curr) => {
            const existingAssetIndex = acc.findIndex(({ metadata }) => SundaeUtils.isAssetIdsEqual(curr.metadata.assetId, metadata.assetId));
        ...
        assets.lovelace = scooperFee + (orderDeposit ?? constants_js_1.ORDER_DEPOSIT_DEFAULT);
    ...
    static getBestPoolBySwapOutcome({ availablePools, given, taken, }) {
        let bestPool;
        let bestOutcome;
        const [resolvedGiven, resolvedTaken] = SundaeUtils.isAdaAsset(given?.metadata)
            ? [given, taken]
            : [taken, given];
        if (availablePools && resolvedGiven?.metadata && resolvedTaken?.metadata) {
            ...
            for (const pool of availablePools) {
                const givenReserve = SundaeUtils.isAssetIdsEqual(pool.assetA.assetId, resolvedGiven.metadata.assetId)
                    ? pool.liquidity.aReserve
                    : pool.liquidity.bReserve;
                const takenReserve = SundaeUtils.isAssetIdsEqual(pool.assetB.assetId, resolvedTaken.metadata.assetId)
                    ? pool.liquidity.bReserve
                    : pool.liquidity.aReserve;
                const swapOutcome = (0, cpp_1.getSwapOutput)(hundredGiven, givenReserve, takenReserve, pool.currentFee);
     ...
     */
    static splitMetadataString(str, prefix) {
        const result = [];
     ...
     * @returns {number}
     */
    static unixToSlot(unix, network) {
        return Math.floor(Math.trunc(Number(unix)) -
            (network === "mainnet"
     ...
     * @returns {number}
     */
    static slotToUnix(unix, network) {
        return Math.floor(Math.trunc(Number(unix)) +
            (network === "mainnet"
                ? SundaeUtils.MAINNET_OFFSET
                : SundaeUtils.PREVIEW_OFFSET));
...
exports.SundaeUtils = SundaeUtils;
//# sourceMappingURL=SundaeUtils.class.js.map