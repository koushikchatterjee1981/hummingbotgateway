 ...
 */
export const getFirstLp = (a, b) => sqrt(a * b);
 ...
 * @returns
 */
export const getShare = (lp, totalLp) => new Fraction(lp, totalLp);
 ...
 */
export const calculateLiquidity = (a, b, aReserve, bReserve, totalLp) => {
 ...
 */
export const addLiquidity = (a, aReserve, bReserve, totalLp) => {
 ...
 */
export const getTokensForLp = (lp, aReserve, bReserve, totalLp) => [new Fraction(lp * aReserve, totalLp).quotient, new Fraction(lp * bReserve, totalLp).quotient];
/**
 ...
 */
export const getSwapOutput = (input, inputReserve, outputReserve, fee, roundOutputUp) => {
  if (input <= 0 || inputReserve <= 0 || outputReserve <= 0) throw new Error("Input and reserves must be positive");
  fee = Fraction.asFraction(fee);
  if (fee.lt(Fraction.ZERO) || fee.gt(Fraction.ONE)) throw new Error("fee must be between 0 and 1");
  ...
  const safeOutput = roundOutputUp ? (outputNumerator + outputDenominator - 1n) / outputDenominator : output.quotient;
  const inputLpFee = new Fraction(input * fee.numerator, fee.denominator).quotient;
 ...
 * @returns The swap details
 */
export const getSwapInput = (output, inputReserve, outputReserve, fee) => {
  ...
  fee = Fraction.asFraction(fee);
  if (fee.lt(Fraction.ZERO) || fee.gte(Fraction.ONE)) throw new Error("fee must be [0,1)");
  const feeDiff = BigInt(fee.denominator - fee.numerator);
  const input = new Fraction(inputReserve * output * fee.denominator, (outputReserve - output) * feeDiff).quotient + 1n;
  const inputLpFee = new Fraction(input * fee.numerator, fee.denominator).quotient;
  const nextInputReserve = inputReserve + input;
 ...
 */
export const getAssetsRatio = (firstValue, secondValue) => {
 ...
 * ```
 */
export function getSwapRatio(direction, assets) {
  ...
  if (direction === "A_PER_B") {
    ...
    calculatedAmount = AssetAmount.fromValue(Number(rawRatio) * 10 ** (secondAsset.decimals - firstAsset.decimals), firstAsset.decimals);
  } else {
    rawRatio = getAssetsRatio(secondAsset.quantity, firstAsset.quantity);
    calculatedAmount = AssetAmount.fromValue(Number(rawRatio) * 10 ** (firstAsset.decimals - secondAsset.decimals), secondAsset.decimals);
  }
  return {
